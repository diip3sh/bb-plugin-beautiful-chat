...
| `app.tsx` | The app overlay (`app.slots.experimental_appOverlay`). Renders nothing; reads settings with `useSettings()`, writes `data-bui-loader`, `data-bui-chips` and `data-bui-off` onto `<html>`, mounts the spatial tooltips when `messageActions` is on, and registers the coins worklet when `loader` is `coins`. Removes all attributes on unmount. |
